  ...
  const handleSave = async (e) => {
    ...
    try {
      if (!navigator.onLine) {
        ...
        toast.success("Guardado OFFLINE", { id: loadingToast });
      } else {
        ...
        toast.success("¡Subido!", { id: loadingToast });
      }
    ...
    <div style={{ padding: '15px', border: '1px solid #eee', borderRadius: '12px', background: 'white', boxShadow: '0 2px 5px rgba(0,0,0,0.05)' }}>
      ...
      <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
        ...
        <div style={{ backgroundColor: '#f8f9fa', padding: '10px', borderRadius: '8px', border: '1px solid #e9ecef' }}>
          ...
          <input placeholder="Buscar..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} style={{width: '100%', padding: '8px', marginBottom: '5px', border: '1px solid #ccc', borderRadius: '6px'}} />

          {/* --- AQUÍ ESTÁ EL DIAGNÓSTICO VISIBLE --- */}
          <div style={{background: '#222', color: '#0f0', padding: '10px', borderRadius: '4px', fontSize: '12px', marginBottom: '10px', fontFamily: 'monospace'}}>
            <strong>DIAGNÓSTICO EN VIVO:</strong><br/>
            📥 Total Descargados de Nube: {students.length}<br/>
            👀 Visibles con Filtros: {visibleStudents.length}<br/>
            <small style={{color: '#ccc'}}>Si Total es 0, Firebase no envía datos.</small><br/>
            <small style={{color: '#ccc'}}>Si Total 0 y Visible es 0, ajusta los filtros de arriba.</small>
          </div>
          {/* --------------------------------------- */}
